function [X] = lyap3c2(A, E, X, TRANS)
%lyap3c2 Solves the 3-way reduced continuous-time generalized Lyapunov equation
%
        ...
        for M = L:-1:1
            ...
            for K = M:-1:1
               ...
               % ~~~~~~~~~~ Update right-hand sides I (eq. 13) ~~~~~~~~~~~
                    TMP = reshape(X(K+1:N, L, M:N),N-K,[]);
                     X(K, K:L, M) = X(K, K:L, M) - ( reshape(A(K, K+1:N) * TMP,[],N-M+1) * E(M, M:N).' ) * E(K:L, L).';
                    TMP = reshape(E(K, K+1:N) * TMP,[],N-M+1);
                     X(K, K:L, M) = X(K, K:L, M) - ( TMP * A(M, M:N).' ) * E(K:L, L).';
                     X(K, K:L, M) = X(K, K:L, M) - ( TMP * E(M, M:N).' ) * A(K:L, L).';
                end
                % ~~~~~~~~~~ Update right-hand sides II (eq. 14) ~~~~~~~~~~
                if M < N
                    TMP = reshape(X(K, L, M+1:N),[],N-M);
                     X(K, K:L, M) = X(K, K:L, M) ...
                        - ( E(K, K) * TMP * A(M, M+1:N).' ) * E(K:L, L).';
                    TMP = TMP * E(M, M+1:N).';
                     X(K, K:L, M) = X(K, K:L, M) - ( A(K, K) * TMP ) * E(K:L, L).';
                     X(K, K:L, M) = X(K, K:L, M) - ( E(K, K) * TMP ) * A(K:L, L).';
                end
