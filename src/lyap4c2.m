function [X] = lyap4c2(A, E, X, TRANS)
%lyap4c2 Solves the 4-way reduced continuous-time generalized Lyapunov equation
%
%   Usage: X = lyap4c2(A,E,Y,TRANS)
%
%   Inputs:
        ...
        for M = L:-1:1
            ...
            for NN = M:-1:1
                ...
                for K = NN:-1:1
                    % ~~~~~~~~~~ Update right-hand sides I ~~~~~~~~~~
                    if K < N
                        TMP = reshape(X(K+1:N,L,M:N,NN:N),N-K,[]); 
                         X(K,K:L,M,NN) = X(K,K:L,M,NN) - ( reshape( reshape( A(K,K+1:N) * TMP ,[],N-NN+1) * E(NN,NN:N).',[],N-M+1) * E(M,M:N).' ) * E(K:L,L).';
                        TMP = reshape( E(K,K+1:N) * TMP ,[],N-NN+1); 
                         X(K,K:L,M,NN) = X(K,K:L,M,NN) - ( reshape( TMP * A(NN,NN:N).',[],N-M+1) * E(M,M:N).' ) * E(K:L,L).';
                        TMP = reshape( TMP * E(NN,NN:N).',[],N-M+1); 
                         X(K,K:L,M,NN) = X(K,K:L,M,NN) - ( TMP * A(M,M:N).' ) * E(K:L,L).';
                         X(K,K:L,M,NN) = X(K,K:L,M,NN) - ( TMP * E(M,M:N).' ) * A(K:L,L).';
                    end
                    % ~~~~~~~~~~ Update right-hand sides II ~~~~~~~~~~~
                    if NN < N
                        TMP = reshape(X(K,L,M:N,NN+1:N),[],N-NN); 
                         X(K,K:L,M,NN) = X(K,K:L,M,NN) - ( reshape( A(K,K) * TMP * E(NN,NN+1:N).',[],N-M+1) * E(M,M:N).' ) * E(K:L,L).';
                        TMP = E(K,K) * TMP; 
                         X(K,K:L,M,NN) = X(K,K:L,M,NN) - ( reshape( TMP * A(NN,NN+1:N).',[],N-M+1) * E(M,M:N).' ) * E(K:L,L).';
                        TMP = reshape( TMP * E(NN,NN+1:N).',[],N-M+1); 
                         X(K,K:L,M,NN) = X(K,K:L,M,NN) - ( TMP * A(M,M:N).' ) * E(K:L,L).';
                         X(K,K:L,M,NN) = X(K,K:L,M,NN) - ( TMP * E(M,M:N).' ) * A(K:L,L).';
                    end
                    % ~~~~~~~~~~ Update right-hand sides III ~~~~~~~~~~
                    if M < N
                        TMP = reshape(X(K,L,M+1:N,NN), [], N-M);
                         X(K,K:L,M,NN) = X(K,K:L,M,NN) - ( A(K,K) * TMP * E(NN,NN).' * E(M,M+1:N).' ) * E(K:L,L).';
                        TMP = E(K,K) * TMP; 
                         X(K,K:L,M,NN) = X(K,K:L,M,NN) - ( (TMP * A(NN,NN).') * E(M,M+1:N).' ) * E(K:L,L).';
                        TMP = TMP * E(NN,NN).'; 
                         X(K,K:L,M,NN) = X(K,K:L,M,NN) - ( TMP * A(M,M+1:N).' ) * E(K:L,L).';
                         X(K,K:L,M,NN) = X(K,K:L,M,NN) - ( TMP * E(M,M+1:N).' ) * A(K:L,L).';
                    end
                    ...
                    if K < L
                        TMP = X(K,L,M,NN); 
                         X(K,K:L-1,M,NN) = X(K,K:L-1,M,NN) - (A(K,K) * TMP * E(NN,NN).' * E(M,M).') * E(K:L-1,L).';
                        TMP = E(K,K) * TMP; 
                         X(K,K:L-1,M,NN) = X(K,K:L-1,M,NN) - (TMP * A(NN,NN).' * E(M,M).') * E(K:L-1,L).';
                        TMP = TMP * E(NN,NN).'; 
                         X(K,K:L-1,M,NN) = X(K,K:L-1,M,NN) - (TMP * A(M,M).') * E(K:L-1,L).';
                         X(K,K:L-1,M,NN) = X(K,K:L-1,M,NN) - (TMP * E(M,M).') * A(K:L-1,L).';
                    end
                end
            end
        end
